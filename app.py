...
def harvest(plant_id):
    ...
    insert_harvest = harvests.insert_one(new_harvest)
...
def edit(plant_id):
    ...
    if request.method == 'POST':
        ...
        newvalues = { "$set": edit_plant }
        update_plant = plants.update_one({"_id": ObjectId(plant_id)},newvalues)
        return redirect(url_for('detail', plant_id=plant_id))
    else:
...
def delete(plant_id):
    ...
    # the given plant id.
    harvests_to_delete = harvests.find({"plant_id":(plant_id)})
    for harvest in harvests_to_delete:
        harvests.delete_one(harvest)
